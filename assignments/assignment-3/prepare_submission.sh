#! /bin/bash
#
# Builds the zip for Gradescope.  run_autograder only copies these six files out
# of a submission and takes everything else from its own checkout, so there is
# no reason to upload the rest -- and the datasets are far too big to upload
# anyway.
...
FILES="
lab/neon/main.c
homework/block_mm_neon/main.c
homework/opencl_naive_mm/main.c
homework/opencl_naive_mm/kernel.cl
homework/opencl_tiled_mm/main.c
homework/opencl_tiled_mm/kernel.cl
"
...
# name anywhere under the upload.
rm -f "$out"
