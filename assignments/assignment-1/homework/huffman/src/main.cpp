...
//     std::cout << "compressed bits ares: ";
//     for (int i = 0; i < code_size; i++)
//     {
//         std::cout << std::bitset<8>(code[i]);
//     }
//     std::cout << std::endl;

// 	unsigned char *out = NULL;
// 	uint32_t out_size = 0;
// 	huffman_decode(code, code_size, &out, &out_size);
//     std::cout << "decompressed bits ares: ";
...
int main(int argc, const char *argv[])
{
	...
	if (ret != 0)
	{
		cerr << "Huffman encode failed with code " << ret << endl;
		return EXIT_FAILURE;
	}
	...
	if (ret != 0)
	{
		cerr << "Huffman decode failed with code " << ret << endl;
		return EXIT_FAILURE;
	}
