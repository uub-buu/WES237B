            ...
            for (int b_k = 0; b_k < input0_width; b_k += block_size)
            {
                ...
                for (int ii = b_i; ii < ((b_i + block_size < input0_height) ? (b_i + block_size) : input0_height); ii++)
                {
                    for (int jj = b_j; jj < ((b_j + block_size < input1_width) ? (b_j + block_size) : input1_width); jj++)
                    {
                        ...
                        int temp2 = 0;
                        int temp3 = 0;
                        for (int kk = b_k; kk < ((b_k + block_size < input0_width) ? (b_k + block_size) : input0_width); kk+=4)
                        {

                            int index_0 = (ii * input0->shape[1]) + kk;
                            int index_1 = (kk * input1->shape[1]) + jj;
                            temp0 += input0->data[index_0] * input1->data[index_1];
                            temp1 += input0->data[index_0+1] * input1->data[index_1+1];
                            temp2 += input0->data[index_0+2] * input1->data[index_1+2];
                            temp3 += input0->data[index_0+3] * input1->data[index_1+3];
                        }
